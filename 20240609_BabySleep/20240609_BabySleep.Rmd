...
```{r data_prep}
background <- readPNG('huck_background.png')

dt <- read_csv('b62171c7-b0c9-428b-a204-8944699629bd.csv') %>% 
  filter(Type == 'Sleep') %>% 
...
skeleton <- tibble(
  ...
  to = ymd_hms('2023-11-10 23:59:00'),
  by = '15 min'
))
...
overall / by_month +
  ...
  plot_annotation(
    ...
    subtitle = glue('*May 22, 2023 - {format(as.Date(max(dt$End)), "%B %d, %Y")}*'),
